...
import { DisplayNameForm, getDisplayName } from './AccountPage';
...
const CommentForm = ({ onCancel, ...props }: CommentFormProps & { onCancel?: () => void }) => {
  ...
  const [submittedComment, setSubmittedComment] = React.useState<CommentT | null>(null);
...
export const Comments = ({ comments, ...props }: CommentsProps) => {
  const [toShow, setToShow] = React.useState<Array<CommentWithOrWithoutRepliesT>>(comments);
  const [displayName, setDisplayName] = React.useState(getDisplayName(props.user));

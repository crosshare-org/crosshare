...
import { fromCells, ViewableGrid, ViewableEntry, addClues } from '../../../lib/viewableGrid';
import { puzzleFromDB, PuzzleT, Direction } from '../../../lib/types';

function layoutPDFClues(doc: jsPDF, puzzle: PuzzleT, grid: ViewableGrid<ViewableEntry>, squareSize: number) {
  ...
  const allClues = acrossTitle.concat(acrossClues).concat(downTitle).concat(downClues);
  for (let i = 0; i < allClues.length; i++) { // Position clue on page
    ...
    if (['across', 'down'].includes(allClues[i].label.toLowerCase())) { // Make Across, Down headings bold
...
function layoutPDFGrid(doc: jsPDF, x: number, y: number, puzzle: PuzzleT, grid: ViewableGrid<ViewableEntry>): number {
  const squareSize = Math.min(24, 360 / puzzle.size.rows, 360 / puzzle.size.cols);
  const format = {
  ...
  doc.rect(format.gridOrigin.x, format.gridOrigin.y,
    puzzle.size.cols * format.squareSize, puzzle.size.rows * format.squareSize, 'D');
  doc.setLineWidth(format.innerLineWidth);
  for (let i = 0; i < puzzle.size.rows; i++) {
    for (let j = 0; j < puzzle.size.cols; j++) {
      doc.setFillColor(puzzle.grid[i * puzzle.size.cols + j] === '.' ? 'black' : 'white');
...
function getPdf(puzzle: PuzzleT): ArrayBuffer {
  ...
  pdf.setProperties({ title: puzzle.title, creator: 'crosshare.org', author: puzzle.authorName });
